...
bool bReportRecyleEvents = true;
bool bReportOEMresync = true;
bool bReportBlueWireData = true;
...
void BlueWireTask(void*) {
  ...
  for (;;) {
    ...
    case CommStates::Idle:
      ...
      if (RxTimeElapsed >= (/* NVstore.getUserSettings().FrameRate */ 1000 - 60)) {  // compensate for the time spent just doing things in this state machine
